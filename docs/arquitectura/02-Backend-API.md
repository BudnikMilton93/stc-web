...
3. Tests automatizados de la API (`api/src/Stc.Api.Tests`, xUnit + `WebApplicationFactory` + Testcontainers), tests del frontend (`frontend/src/**/*.test.{js,jsx}`, Vitest + React Testing Library) y una revisión de seguridad ya están hechos (ver [../roadmap-fortalecimiento.md](../roadmap-fortalecimiento.md) para el detalle), igual que CI básico (`.github/workflows/ci.yml`, lint+test+build del frontend y build+tests de la API en cada push/PR). Queda pendiente e2e de flujos críticos (Playwright), antes de operar con datos reales de producción.
