...
- `UnhandledExceptionHandlerTests.cs` — el `IExceptionHandler` centralizado (ver [02-Backend-API.md](02-Backend-API.md)): una excepción no controlada (vía `/test/throw`, mapeado solo en `Testing`) queda registrada en `log_errores` y responde 500 + `ProblemDetails` consistente con lo que espera `ApiError` en `frontend/src/lib/apiClient.js`; un request anónimo a un endpoint protegido no llega al handler; el `usuario_id` queda nulo cuando la excepción ocurre sin sesión; un error de negocio ya manejado (400 de validación, 409 por constraint único) no genera ninguna entrada de log; no persiste cambios trackeados no guardados por el endpoint que falló (`db.ChangeTracker.Clear()`); una violación de constraint único devuelve 409 (vía `DbSaveExtensions.TrySaveChangesAsync`) y tampoco genera log.
