...
| 5 | Pipeline de deploy a Azure | Job agregado (2026-09-07), pendiente de validar con un push real | `deploy-api` en `.github/workflows/ci.yml`, autenticado contra Azure vía OIDC (sin secrets de larga duración) |
...
### 5. Pipeline de deploy a Azure — Job agregado (2026-09-07), pendiente de validar con un push real
...
Dos errores de configuración que aparecieron en el camino y valen la pena documentar porque son fáciles de repetir: (a) el rol se asignó primero al usuario personal en vez de al App Registration (`gh-actions-stc-web`) — el miembro de la asignación tiene que ser la entidad de servicio, no una persona; (b) el rol se asignó primero con scope en el App Service Plan (`serverFarms/ASP-stcwebrg-aa92`) en vez de en el sitio (`sites/stc-api`) — con ese scope el pipeline no habría podido tocar el recurso real.

Pendiente: hacer un push a `main` para validar el pipeline completo de punta a punta por primera vez.
