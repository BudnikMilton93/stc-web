...
| 12 | Definir dónde y cómo se hospeda la API en producción | Pendiente, a evaluar | Hoy la API corre en `localhost` y contra remoto habla con el pooler de Supabase a través de internet (cada round trip paga latencia de red real). Mitigado parcialmente (2026-09-06) con tuning de pool de conexiones (`Minimum Pool Size`/`Keepalive` en `ServiceCollectionExtensions.cs`) para no repagar el handshake TCP+TLS en cada rato de inactividad — no reemplaza hospedar la API cerca de la región de Supabase, que es la solución de fondo al costo de round trip |
