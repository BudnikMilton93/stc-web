...
`api/src/Stc.Api.Tests` tiene tests de la API (xUnit + `WebApplicationFactory`, Postgres real vía Testcontainers — correr con `dotnet test` desde `api/`). `frontend/src/**/*.test.{js,jsx}` tiene tests del frontend (Vitest + React Testing Library — correr con `npm run test` desde `frontend/`). `frontend/e2e/` tiene tests end-to-end (Playwright) del flujo crítico login → cliente → sitio → unidad → ocupante → activo, contra el sistema real (Supabase local + API real + frontend real, sin mocks) — correr con `npm run test:e2e` desde `frontend/` (requiere `supabase start` corriendo; levanta la API y el frontend automáticamente). Deliberadamente fuera de CI: es lento y depende de 3 procesos coordinados, se corre a mano antes de cambios grandes o de un deploy. CI (`.github/workflows/ci.yml`, GitHub Actions) corre en cada push/PR: job `frontend` (`npm ci` → lint → test → build) y job `api` (`dotnet restore` → build Release → test Release contra Postgres real). Ver `docs/roadmaps/00-fortalecimiento.md` para el estado de esta y otra deuda técnica conocida.
